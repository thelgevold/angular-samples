...
import {IPerson, IPersons} from '../../models';
...
export class AddressBook implements OnInit {
  ...
  ngOnInit() {
    this.res = this.addressBookService.getEntries().pipe(map((persons: Array<IPerson>) => {
      // IPerson is generated by Bazel from person.proto
      return persons.map((person: IPerson) => ({firstName: person.name, streetAddress: person.address}))
     }))
